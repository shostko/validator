...
): DelegatedValidator<Any, T, R> = CompositeValidator(*validators)

fun <T : Any?, RIN : Any?, ROUT : Any?> Validator<T, RIN>.map(
    mapper: suspend (RIN) -> ROUT,
): DelegatedValidator<Any, T, ROUT> = MappedValidator(this, mapper)
